...
describe('getCompanyLogoUrl', () => {
  it('builds a logo url from company website', () => {
    expect(getCompanyLogoUrl('https://www.stripe.com', '')).toBe('https://icons.duckduckgo.com/ip3/stripe.com.ico');
  });

  it('returns resilient favicon candidates in priority order', () => {
  ...
  it('falls back to external link only for non-job-board domains', () => {
    expect(getCompanyLogoUrl(undefined, 'https://careers.stripe.com/openings')).toBe(
      'https://icons.duckduckgo.com/ip3/careers.stripe.com.ico'
    );
  });
